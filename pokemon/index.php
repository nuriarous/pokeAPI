...
<body>
    ...
    <div id="iniciales">
        ...
        // Obtener el número de resultados por página y la página actual desde la URL
        $limit = isset($_GET['limit']) ? $_GET['limit'] : 20;
        $page = isset($_GET['page']) ? $_GET['page'] : 1;

        // Calcular el offset para la solicitud a la API
        $offset = ($page - 1) * $limit;
    ...
    <div class="abajo">
        <?php
        if ($page > 1) {
            echo '<a href="?page=' . ($page - 1) . '&limit=' . $limit . '">Anterior</a>&nbsp;&nbsp ';
        }

        if (count($valores['results']) == $limit) {
            echo '<a href="?page=' . ($page + 1) . '&limit=' . $limit . '">Siguiente</a>&nbsp;&nbsp<br>';
        }
        if (count($valores['results']) == $limit) {
            echo '<a href="?page=' . ($page = 1) . '&limit=' . $limit . '">INICIO</a>';
        }
        ?>
    </div>
